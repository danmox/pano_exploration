...
void Panorama::captureLoop()
{
    ...
    while (frame <= number_of_frames) {
        ...
        if (sgn(headingDifference(capture_angles[frame])) != sgn(spin_speed)) {
            ...
            KDL::Frame t_camera_world = t_odom_world*t_robot_odom*t_camera_robot;
            tf2::Stamped<KDL::Frame> trans_stamped(t_camera_world, hd.stamp, world_frame);
            geometry_msgs::PoseStamped camera_pose;
            tf2::convert(trans_stamped, camera_pose);
